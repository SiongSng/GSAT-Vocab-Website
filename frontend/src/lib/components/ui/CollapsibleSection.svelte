<script lang="ts">
    ...
    interface Props {
        title: string;
        icon?: "chart" | "puzzle" | "warning" | "link";
        defaultOpen?: boolean;
        children: Snippet;
    }
...
<div class="collapsible-section">
    ...
    >
        <div class="flex items-center gap-2">
            ...
            {/if}
            <h3 class="text-sm font-semibold text-section-header">{title}</h3>
        </div>
        <svg
            class="w-4 h-4 text-content-tertiary transition-transform duration-200 {isOpen
                ? 'rotate-180'
                : ''}"
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
