<script lang="ts">
    ...
    const isSheetOpen = $derived(lookup.isMobileOpen && lookup.position === "bottom-sheet");

    async function playAudio() {
        if (!item?.entry || isPlaying) return;
        isPlaying = true;
        try {
            const url = await speakText(item.entry.lemma);
...
<BottomSheet isOpen={isSheetOpen} onClose={closeLookup}>
    ...
    {:else if item?.entry}
        <div class="sheet-content">
            ...
            <div class="flex items-center gap-1 text-sm text-content-tertiary mb-4">
                {#if primarySense}
