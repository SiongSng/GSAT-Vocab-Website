...
if echo "$kv_output" | grep -q "id"; then
    ...
    # 更新 backend/api/wrangler.toml
    print_info "更新 backend/api/wrangler.toml 中的 KV namespace ID..."
    if [[ "$OSTYPE" == "darwin"* ]]; then
        # macOS
        sed -i '' "s/^id = \".*\"/id = \"$kv_id\"/" backend/api/wrangler.toml
        sed -i '' "s/^preview_id = \".*\"/preview_id = \"$kv_preview_id\"/" backend/api/wrangler.toml
    else
        # Linux
        sed -i "s/^id = \".*\"/id = \"$kv_id\"/" backend/api/wrangler.toml
        sed -i "s/^preview_id = \".*\"/preview_id = \"$kv_preview_id\"/" backend/api/wrangler.toml
    ...
    print_success "backend/api/wrangler.toml 已更新"
else
    print_warning "KV namespace 可能已存在，請手動檢查 backend/api/wrangler.toml"
fi
}
...
deploy_workers() {
    ...
    if [ ! -f "backend/api/worker-api.js" ]; then
        print_error "backend/api/worker-api.js 不存在"
        return 1
    fi

    if [ ! -f "backend/api/wrangler.toml" ]; then
        print_error "backend/api/wrangler.toml 不存在"
        return 1
    fi
    ...
    worker_url=$(wrangler deployments list --config backend/api/wrangler.toml 2>&1 | grep -oP 'https://[^ ]+' | head -1 || echo "")

    if [ -n "$worker_url" ]; then
        print_success "Worker URL: $worker_url"
        echo ""
        print_warning "請將此 URL 更新到 frontend/app.js 中的 CONFIG.API_BASE"
    fi
}
...
build_frontend() {
    ...
    if [ ! -f "frontend/index.html" ] || [ ! -f "frontend/app.js" ]; then
        print_error "前端文件不存在"
        return 1
    fi
    ...
    # 複製文件
    cp frontend/index.html "$DEPLOY_DIR/index.html"
    cp frontend/app.js "$DEPLOY_DIR/app.js"

    # 如果有其他靜態資源，也複製
    if [ -d "frontend/assets" ]; then
        cp -r frontend/assets "$DEPLOY_DIR/"
    fi
